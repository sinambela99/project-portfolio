        ...
        <div className="flex min-w-[820px] items-center gap-3">
          {nodes.map((node, index) => (
            <div key={node.name} className="flex items-center gap-3">
              <button type="button" onMouseEnter={() => setActive(node)} onClick={() => setActive(node)} className="h-28 w-32 rounded-md border border-white/10 bg-white/[0.03] p-3 text-center text-sm font-semibold text-slate-100 transition hover:border-sky-300/50 hover:bg-sky-300/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-300">
                {node.name}
              </button>
              {index < nodes.length - 1 ? <span className="h-px w-10 bg-sky-300/40" aria-hidden /> : null}
            </div>
          ))}
        </div>
      </div>
      <aside className="rounded-lg border border-white/10 bg-[#081421] p-6">
        <p className="font-mono text-sm uppercase tracking-[0.18em] text-sky-200">Komponen aktif</p>
        <h3 className="mt-3 text-2xl font-semibold text-slate-50">{active.name}</h3>
        <p className="mt-4 leading-8 text-slate-300">{active.detail}</p>
        <div className="mt-6 rounded-md border border-white/10 bg-slate-950/70 p-4 font-mono text-sm leading-7 text-slate-300">
